...
test.describe('TC0100 参数设置管理', () => {
  const testName = `测试参数_${Date.now()}`;
  const testKey = `test.param.${Date.now()}`;

  test('TC0100a: 页面加载并展示数据列表', async ({ adminPage }) => {
  ...
  test('TC0100b: 按参数名称搜索', async ({ adminPage }) => {
    const configPage = new ConfigPage(adminPage);
    await configPage.goto();
  ...
  test('TC0100c: 按参数键名搜索', async ({ adminPage }) => {
    const configPage = new ConfigPage(adminPage);
    await configPage.goto();
  ...
  test('TC0100d: 重置搜索条件', async ({ adminPage }) => {
    const configPage = new ConfigPage(adminPage);
    await configPage.goto();
  ...
  test('TC0100e: 创建新参数设置', async ({ adminPage }) => {
    const configPage = new ConfigPage(adminPage);
    await configPage.goto();
  ...
  test('TC0100f: 新创建的参数可搜索到', async ({ adminPage }) => {
    const configPage = new ConfigPage(adminPage);
    await configPage.goto();
  ...
  test('TC0100g: 编辑参数设置', async ({ adminPage }) => {
    const configPage = new ConfigPage(adminPage);
    await configPage.goto();
  ...
  test('TC0100h: 删除参数设置', async ({ adminPage }) => {
    const configPage = new ConfigPage(adminPage);
    await configPage.goto();
  ...
  test('TC0100i: 导出按钮功能正常', async ({ adminPage }) => {
    const configPage = new ConfigPage(adminPage);
    await configPage.goto();
