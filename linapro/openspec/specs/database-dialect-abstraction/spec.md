...
定义宿主数据库方言抽象层，用于统一收敛 MySQL 与 SQLite 等数据库引擎差异。

## 需求

### 需求：宿主必须通过统一的方言抽象层收敛数据库引擎差异
...
#### 场景：dialect 公共包不暴露宿主 internal 具体类型
...
### 需求：SQLite 方言 DDL 转译必须覆盖项目 DDL 子集
...
#### 场景：表内联索引被提取为独立 CREATE INDEX 语句
...
#### 场景：CREATE DATABASE 与 USE 整句被丢弃
