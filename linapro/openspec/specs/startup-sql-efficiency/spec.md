...
定义宿主启动期 SQL 明细日志、启动快照复用、插件清单 no-op 同步、内置定时任务启动注册和启动效率回归测试的治理边界。
...
- **当** 使用交付默认配置启动后端服务
- **则** `database.default.debug` 为 `false`
- **且** 启动日志不得包含 `SHOW FULL COLUMNS`、`SELECT ... FROM`、`INSERT INTO` 等 ORM SQL 明细行

#### Scenario: 显式开启 SQL debug
- **当** 管理员将 `database.default.debug` 设置为 `true`
- **则** 后端服务允许输出 ORM SQL 明细
- **且** 该行为仅作为诊断模式，不作为默认开发或生产配置
...
- **当** 后端执行一次 HTTP 启动编排
- **则** 插件 catalog 启动快照最多构造一次
- **且** 后续插件启动阶段复用该快照读取 `sys_plugin` 与 `sys_plugin_release`

#### Scenario: 同一次启动只构造一次插件 integration 快照
- **当** 后端执行一次 HTTP 启动编排
- **则** 插件 integration 启动快照最多构造一次
- **且** 后续菜单和资源引用同步复用该快照读取 `sys_menu` 与 `sys_plugin_resource_ref`

#### Scenario: 启动写入同步更新快照
- **当** 启动同步阶段创建或更新插件 registry、release、menu 或 resource ref 投影
- **则** 系统必须同步更新当前启动快照
- **且** 后续启动阶段不得为了读取刚写入的投影再次全表扫描
...
- **当** 源码插件 manifest 与数据库中 registry、release、菜单、权限和资源引用投影完全一致
- **则** 启动同步该插件时不得执行 `INSERT`、`UPDATE`、`DELETE`
- **且** 不得产生空的 `BEGIN` / `COMMIT` 事务

#### Scenario: 菜单声明变更时才进入菜单事务
- **当** 插件 manifest 的菜单或动态 route permission 发生变化
- **则** 系统可以开启菜单同步事务并写入必要变更
- **且** 事务提交后必须更新启动快照中的菜单投影

#### Scenario: 发布快照变更时才同步 release metadata
- **当** 插件 manifest 生成的 release snapshot 与当前 `sys_plugin_release` 行一致
- **则** 系统不得更新 release metadata
- **且** 不得为了 refresh release snapshot 再次查询同一 release 行
...
- **当** 后端启动并同步内置任务投影
- **则** 系统使用同步返回的 projection snapshot 注册内置任务
- **且** 注册过程不需要按 ID 重新读取同一内置任务行

#### Scenario: 持久化扫描跳过内置任务
- **当** 持久化调度器执行启动加载
- **则** 查询条件必须排除 `is_builtin=1` 的任务
- **且** 只加载用户创建的启用任务或非内置插件任务
...
- **当** 后端完成插件启动同步
- **则** 日志输出插件扫描数量、发生变更的插件数量和 no-op 插件数量
- **且** 日志不得包含完整 SQL 语句文本

#### Scenario: 启动摘要包含快照构造次数
- **当** 后端完成 HTTP 启动编排
- **则** 日志输出 catalog、integration 和 job 启动快照构造次数
- **且** 同一启动阶段的重复快照构造会被测试或审查识别为回归
...
- **当** 测试使用默认数据库 debug 配置启动后端服务
- **则** 测试断言启动日志不包含 ORM SQL 明细
- **且** 测试断言启动摘要日志存在

#### Scenario: 插件 no-op 同步回归测试
- **当** 测试准备一个已同步且无 manifest 差异的插件
- **则** 再次执行启动同步不得产生写入 SQL
- **且** 不得产生空事务

#### Scenario: 启动快照复用回归测试
- **当** 测试执行一次 HTTP 启动编排或等价启动编排单元
- **则** catalog、integration 和 job 启动快照构造次数必须分别保持在预算内
- **且** 超出预算时测试失败
