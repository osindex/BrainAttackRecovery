...
`make init` 执行 seed SQL 时 SHALL 全部写入 `tenant_id = 0`(PLATFORM);包括 admin 用户、平台超管角色、内置字典/配置等。

#### Scenario: 初始化数据归属 PLATFORM
- **WHEN** 执行 `make init`
- **THEN** 所有 seed 行 `tenant_id = 0`
- **AND** 所有索引 `(tenant_id, ...)` 联合形态前导列均为 0
