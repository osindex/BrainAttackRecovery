...
- **THEN** operlog `(tenant_id=A, user_id=U, action_kind='biz')`

### Requirement: 强制操作单独审计类
平台管理员通过 `--force` 通道执行的操作 SHALL 写入 `action_kind='platform_force_action'`;载荷包含被绕过的所有 reason key 与上下文(plugin_id 或 tenant_id)。

#### Scenario: force-uninstall 审计
- **WHEN** 平台管理员强制卸载某插件
- **THEN** operlog 写入 `action_kind='platform_force_action', payload={plugin_id, bypassed_reasons:[...]}`
- **AND** 该日志即使在普通查询接口也清晰可见

### Requirement: lifecycle_guard 调用单独审计类
所有 LifecycleGuard 钩子调用 SHALL 在 operlog 中以 `action_kind='lifecycle_guard'` 记录,载荷含 `plugin_id`、`hook_method`、`ok`、`reason`、`elapsed_ms`、`timeout`、`panic` 标志。
...
- **THEN** 在 `/platform/oper-log` 按 `action_kind='platform_force_action' AND created_at >= 上周一` 查询
- **AND** 返回所有 force 操作清单
